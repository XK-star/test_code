...
Gimbal_MoveMode_t    Little_Gimbal_MoveMode;
...
RemoteSwitch_t switch2;   //遥控器右侧拨杆
...
void WorkstateFSM(void)//state只是一个类似的监督机制，attack_mode 是真正的控制最上端。
{
	lastWorkstate  = WorkState;
		if(Little_Gimbal_MoveMode!=	Gimbal_Stop&&RC_CtrlData.rc.s2 == REMOTE_SWITCH_VALUE_UP)
		WorkState=STOP_STATE;
		else if(Little_Gimbal_MoveMode!=	Gimbal_Stop&&RC_CtrlData.rc.s2 == REMOTE_SWITCH_VALUE_CENTRAL)
	...
	switch(WorkState)
	{
		case PREPARE_STATE:
		{		
			if(switch2.switch_value2==REMOTE_SWITCH_CHANGE_2TO3TO1)
			{Little_Gimbal_MoveMode=Gimbal_RC_Mode;
			  WorkState=STOP_STATE;}
		...
		case STOP_STATE :
		{
			if(GetInputMode()!=STOP&&Little_Gimbal_MoveMode==Gimbal_Stop)
...
//			}
	...
	{
				if(RC_CtrlData.rc.s1 == REMOTE_SWITCH_VALUE_UP)
				{
					Attack_Mode = Attack_CatWalk;
				}
				else if(RC_CtrlData.rc.s1 == REMOTE_SWITCH_VALUE_CENTRAL)
				{
					Attack_Mode = Attack_Normal;
				}
				else if(RC_CtrlData.rc.s1 == REMOTE_SWITCH_VALUE_DOWN)
				{
					Attack_Mode = Attack_Normal;
				}
	}
...
/**************************************小云台*****************************************/
void LittleGimbalModeSelect(void)
{
	if(Little_Gimbal_MoveMode!=Gimbal_Stop&&GetWorkState()!=STOP_STATE )
		Little_Gimbal_MoveMode=Gimbal_Stop;
}
	  ...
	  case NORMAL_RC_STATE:
		{
			...
			if(GetInputMode() == REMOTE_INPUT)
			{
				...
				{		
				 ...
				 if(Get_Time_Micros() - rctime_stamp  > 500000)       //枪管热量延迟500ms
					{
						if(GetFrictionState() == FRICTION_WHEEL_ON)
						{
							
							if(switch1.switch_value1== REMOTE_SWITCH_CHANGE_3TO2&&BIG_shoot)
							{
								Shoot_Single(1);
...
//						}							
			...
			else if(GetInputMode() == KEY_MOUSE_INPUT)
			{
				if (GetFrictionState() == FRICTION_WHEEL_ON)		//普通模式 单环速度控制 
				{
					...
					if(RC_CtrlData.mouse.press_l == 1 && (Get_Time_Micros() - time_stamp > 500000)&&Remote_CheckJumpKey(KEY_Q) == 1&&BIG_shoot)     //枪管热量延迟500ms
					{
						time_stamp = Get_Time_Micros();
						set_shootnum(2);
					}

					else if(RC_CtrlData.mouse.press_l == 1 && (Get_Time_Micros() - time_stamp > 500000)&&BIG_shoot)     //枪管热量延迟500ms
					{
						time_stamp = Get_Time_Micros();
						Shoot_Single(1);
						//BulletNumber ++;
					}
					static uint8_t e_state;
					if(e_state==0&&Remote_CheckJumpKey(KEY_R))
							Bullter_Single(1);
					e_state=Remote_CheckJumpKey(KEY_R);
...
void StatusMachine_Update(void)
{
	  ...
	  GetRemoteSwitchAction(&switch2, RC_CtrlData.rc.s2);	
  	WorkstateFSM();
  	Attack_Mode_Select();
  	GimbalModeSelect();
  	ChassisModeSelect();
